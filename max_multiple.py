...
def find_greatest_product(number_string, adjacent_digits=13):
    ...
    while digit_count+adjacent_digits < len(number_string):
        ...
        temp_sum = 0
        # add through the list
        position = 0
        while position < adjacent_digits:
            temp_sum += int(sum_check[position])
            position += 1
        #compare to max and replace if greater
        if(temp_sum > max_product): max_product = temp_sum
        digit_count += 1
